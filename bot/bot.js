...
bot.action(/join_bet_(.+)/, (ctx) => {
    ...
    ctx.reply(
        ...
        `Get ready to battle!`,
        {
            reply_markup: {
                inline_keyboard: [[
                    { text: '⚔️ Start Battle', web_app: { url: `https://gear-wars.vercel.app/?game=${gameId}` } }
                ]]
            }
        }
    );
...
bot.command('leaderboard', (ctx) => {
  ...
  if (topPlayers.length === 0) {
    return ctx.reply('🏆 No players on leaderboard yet! Be the first!');
  }
  
  let leaderboardText = '🏆 Top Players:\n\n';
  topPlayers.forEach((user, index) => {
    const netWins = user.wins - user.losses;
    leaderboardText += `${index + 1}. ${netWins > 0 ? '+' : ''}${netWins} (${user.wins}W/${user.losses}L)\n`;
  });
